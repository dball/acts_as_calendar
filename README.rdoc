...
3. Create an event, e.g. event = calendar.events.create

4. Populate the event with occurrences, e.g. event.occurrences << calendar.dates.find_by_value('2008-01-01')

5. Populate the event with recurrences, e.g. event.recurrences.create({ :weekday => 1 })
